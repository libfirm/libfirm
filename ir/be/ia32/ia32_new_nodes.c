...
/**
 * Returns the argument register slots of an ia32 node.
 */
...
const char *get_ia32_in_reg_name(const ir_node *node, int pos) {
  ...
  assert(pos < get_irn_arity(node) && "Invalid IN position.");

  return attr->in[pos]->name;
}
