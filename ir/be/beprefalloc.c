...
static void check_defs(ir_nodeset_t const *const live_nodes, float const weight, ir_node *const node, arch_register_req_t const *const req)
{
	...
	if (req->same_as != BE_NOT_SAME) {
		ir_node           *const insn   = skip_Proj(node);
		allocation_info_t *const info   = get_allocation_info(node);
		float              const factor = 1.0f / (1 + req->same_as_next);
		for (unsigned i = req->same_as, last = i + req->same_as_next; i <= last; ++i) {
			ir_node *const op = get_irn_n(insn, i);
...
static void assign_reg(ir_node const *const block, ir_node *const node, arch_register_req_t const *const req, unsigned *const forbidden_regs)
{
	...
	if (req->same_as != BE_NOT_SAME) {
		float weight = (float)get_block_execfreq(block);
		for (unsigned i = req->same_as, last = i + req->same_as_next; i <= last; ++i) {
			ir_node *const in        = get_irn_n(in_node, i);
			unsigned const reg_index = arch_get_irn_register(in)->index;
