...
{
	return ir_target.object_format == OBJECT_FORMAT_MACH_O;
}
...
static void emit_section_elf_coff(be_gas_section_t const section, ir_entity const *const entity)
{
	...
	/* section type */
	if (ir_target.object_format != OBJECT_FORMAT_PE_COFF)
...
void be_gas_emit_function_epilog(ir_entity const *const entity)
{
	...
	if (ir_target.object_format == OBJECT_FORMAT_ELF) {
...
{
	return ir_target.object_format == OBJECT_FORMAT_ELF;
}
